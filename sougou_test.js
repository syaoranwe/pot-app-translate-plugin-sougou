...
(async () => {
    const tauriFetch = async (url, options) => {
        ...
        const response = await axios({
            ...
            data: options.body ? options.body.payload : null,
    ...
    try {
        const text = "Find definitions and references for functions and other symbols in this file by clicking a symbol below or in the code.";
        const from = "en";
        const to = "zh";
        const options = {
            utils: {
...
const testCases = [
  {
      ...
      to_lang: 'zh-CHS',
      text: 'Hello, world!',
      expected: '5dceb453616dc53666acac88d63b17cd'  // 假设的预期签名
  },
  {
      from_lang: 'fr',
      to_lang: 'de',
      text: 'Bonjour tout le monde',
      expected: '4ed0a07b9be52fe86f8c9ff4c6170152'  // 假设的预期签名
  },
  {
      from_lang: 'es',
      to_lang: 'ja',
      text: 'Hola, ¿cómo estás?',
      expected: '9c83c4cd11699f50d18c2e7e7b3d531d'  // 假设的预期签名
  }
];

testCases.forEach(({ from_lang, to_lang, text, expected }, index) => {
  const result = calSign(from_lang, to_lang, text);
